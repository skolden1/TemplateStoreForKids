...
namespace MallarEmelieMVC.Areas.Identity.Pages.Account.Manage
{
    public class IndexModel : PageModel
    {
        ...
        [BindProperty] public IFormFile ProfilePictureUpload { get; set; }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            // Hantera profilbild-uppdatering
            if (ProfilePictureUpload != null && ProfilePictureUpload.Length > 0)
            {
                ...
                var uniqueFileName = Guid.NewGuid().ToString() + Path.GetExtension(ProfilePictureUpload.FileName);
                var filePath = Path.Combine(uploadsFolder, uniqueFileName);

                using (var fileStream = new FileStream(filePath, FileMode.Create))
                {
                    await ProfilePictureUpload.CopyToAsync(fileStream);
                }
